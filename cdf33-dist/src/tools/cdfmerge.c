...
static int sp1gAttrs = 0, sp2gAttrs = 3, sp3gAttrs = 6, p1vAttrs = 25,
           sp2vAttrs = 11;
static char *special1gAttrs[1];
/*
...
static char outputCDF[CDF_PATHNAME_LEN+1], fileList[CDF_PATHNAME_LEN+1],
       line[CDF_PATHNAME_LEN+1], sourceCDF[CDF_PATHNAME_LEN+1],
       masterFile[CDF_PATHNAME_LEN+1], mytmp[CDF_PATHNAME_LEN+1];
...
static int  i, ii, j, k, ir, icount, numCDFs, ix, iy, dimN, recNum;
static int numOpt, numTokens = 0, count;
static Logical qopError = FALSE;
static QOP *qop;
static Byte **handles[2], *buffer1, *buffer2;
  ...
  default:
    ...
    ************************************************************************/
    if (qop->qualEntered[MASTERqual]) {
      master = TRUE;
      dataOnly = TRUE;
      strcpyX (masterFile, qop->qualOpt[MASTERqual], CDF_PATHNAME_LEN);
    }
    /************************************************************************
  ...
  for (icount = 0; icount < numCDFs; icount++) {
    ...
    if (icount == 0) {  
      ...
      status = CDFlib (SELECT_, CDF_, ido,
                       CREATE_, ATTR_, "FileSource", GLOBAL_SCOPE, &attrNum,
                       NULL_);
      if (status < CDF_OK) QuitCDF ("3.0", status, "FileSource");
    ...
    /*******************************************************/
    /* Add a file entry to "FileSource" attribute.
    /*******************************************************/
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     NULL_);
    if (status < CDF_OK) QuitCDF ("9.0", status, "FileSource");
...
/*
        if ((strcmpIgCase (varName, "epoch") != 0) ||
            (strcmpIgCase (varName, "range_epoch") != 0)) {
*/
        if (strcmpIgCase (varName, "range_epoch") != 0) {
          if (first) {
            /*******************************************************/
...
*/
        ...
        if (status < CDF_OK) {
          if (status == VAR_EXISTS) {
            if (dataOnly || (cdaweb && 
                             ((strcmpIgCase (newVarName, "EPOCH") != 0) ||
                              (strcmpIgCase (newVarName, "RANGE_EPOCH") != 0))))
              ;
            else
    ...
    if (numvAttrs > 0) {
      ...
      for (i = 0; i < (int) numAttrs; i++) {
        ...
        if (augmentlabel) {
          if ((strcmpIgCase (attrName, "augment_labl") != 0)) 
...
{ 
  ...
  char fileNaming[CDF_PATHNAME_LEN+1];
  char dataVersion[21];
  char **fields;
  char *ptr;
  ...
  for (ix = 0; ix < numgAttrs; ix++) {
    if (strcmpIgCase (attrs[ix], "file_naming_convention") != 0) {
      found = TRUE;
      iy = ix;
      break;
    }
...
{ 
  ...
  if (found) {
    ...
    if (counts > 0) {
      ...
      found = FALSE;
      for (ix = 0; ix < lineAttrs; ix++) {
        if (strcmpIgCase (attrs[ix], "data_version") != 0) {
          found = TRUE;
          iy = ix;
          break;
        }
...
{ 
  ...
  for (ix = 0; ix < lineAttrs; ix++) {
    if (strcmpIgCase (attrs[ix], "Logical_file_id") != 0) {
      found = TRUE;
      iy = ix;
      break;
    }
  ...
  found = FALSE;
  for (ix = 0; ix < lineAttrs; ix++) {
    if (strcmpIgCase (attrs[ix], "Source_name") != 0) {
      found = TRUE;
      iy = ix; 
      break;
    }   
  ...
  found = FALSE;
  for (ix = 0; ix < lineAttrs; ix++) {
    if (strcmpIgCase (attrs[ix], "Descriptor") != 0) {
      found = TRUE;
      iy = ix;
      break;
    }
  ...
  found = FALSE;
  for (ix = 0; ix < lineAttrs; ix++) {
    if (strcmpIgCase (attrs[ix], "Data_type") != 0) {
      found = TRUE;
      iy = ix;
      break;
    }
...
/*
      if ((strcmpIgCase (varNames[iz], "epoch") != 0) ||
          (strcmpIgCase (varNames[iz], "range_epoch") != 0)) {
*/
      if (strcmpIgCase (varNames[iz], "range_epoch") != 0) {
        iy = iz;
        found = TRUE;
        break;
      }
...
{         
  ...
  for (i = 0, ii = 0; ii < (int) numgAttrs; i++) {
    ...
    if (strcmpIgCase (attrName, "File_naming_convention") != 0 &&
        numgEntries == 0)
      fnc = TRUE;
    if (strcmpIgCase (attrName, "Logical_file_id") != 0 && 
        numgEntries == 0)
      lfi = TRUE;
    if (strcmpIgCase (attrName, "Logical_source") != 0 &&
        numgEntries == 0)
      lsi = TRUE;
    ...
    if (dataOnly) 
      ...
      ifound = FALSE;
      if (usePrefix) {
        for (j = 0; j < sp1gAttrs; j++) {
          if (strcmpIgCase (attrName, special1gAttrs[j]) != 0) {
            ifound = TRUE;
            break;
          }
        }
...
/*          if (!master) {  */
            for (ir = 0; ir < sp2gAttrs; ir++) {
              if (strcmpIgCase (newAttrName, special2gAttrs[ir]) != 0) {
                ifound = TRUE;
                break;
              }
            }
          ...
          if (!ifound) {
            if (icount == 0)
              ...
              for (ir = 0; ir < sp3gAttrs; ir++) {
                if (strcmpIgCase (newAttrName, special3gAttrs[ir]) != 0) {
                  ifound = TRUE;
                  break;
                }
              }
              if (!ifound)
                ...
                NULterminateMAX (value, (size_t) numElemsX);
                NULterminateMAX (valueY, (size_t) numElemsY);
            ...
            if (strcmpIgCase (newAttrName, "Logical_source") != 0) {
              int aa, bb;
              aa = 0;
              ...
              status = CDFlib (PUT_, gENTRY_DATA_, CDF_CHAR, numE,                 
                                     outputCDF,
                               NULL_);
            } else if (strcmpIgCase (newAttrName, "Logical_file_id") != 0) {
              if (EndsWithIgCase (outputCDF, ".cdf"))
                numE = (long) strlen (outputCDF) - 4;
              else
                numE = (long) strlen (outputCDF);
...
{         
  ...
  for (i = 0; i < lineAttrs; i++) {
    if (strcmpIgCase (attrs[i], "File_naming_convention") != 0) {
      if (inentries[i] == -1)
        fnc = TRUE;
    } else if (strcmpIgCase (attrs[i], "Logical_file_id") != 0) {
      if (inentries[i] == -1)
        lfi = TRUE;
    } else if (strcmpIgCase (attrs[i], "Logical_source") != 0) {
      if (inentries[i] == -1)                                 
        lsi = TRUE;                                           
    }
    status = CDFlib (SELECT_, CDF_, ido,
    ...
    for (l = 0; l < ii; l++) {
      ifound = FALSE;
      for (ir = 0; ir < sp2gAttrs; ir++) {
        if (strcmpIgCase (attrs[i], special2gAttrs[ir]) != 0) {
          ifound = TRUE;
          break;
        }
      }
      if (!ifound) {
        ...
        if (strcmpIgCase (attrs[i], "Logical_source") != 0) {
          int aa, bb;
          aa = 0;
          for (bb = (int) strlen (outputCDF)-1; bb >= 0; bb--) {
            if (outputCDF[bb] == '_') {
...
char *masterFile;
{
  ...
  char line[MAX_MERGELINE_LEN+1+1], item[MAX_MERGELINE_LEN+1];
  char *ptr, *ptr2;
  char **tmp;
  int  *tmp2;
  Logical ib;
  ...
  ib = FALSE;
  while (fgets(line,MAX_MERGELINE_LEN,masterfp) != NULL) {
    if (sscanf(line, "%s\n", item) != 1) continue;
    ...
    ptr = strchr (line, ':');
    ptr2 = strchr (line, ' ');
    if (ptr != NULL && ptr2 == NULL && line[(int)strlen(line)-1] == ':') { 
      if (ib == TRUE) {
      ...
      attrs[lineAttrs] = (char *) cdf_AllocateMemory (strlen(line)+1, FatalError);
      strcpyX (attrs[lineAttrs], line, strlen(line)-1);
      lineAttrs++;
      ib = TRUE;
    } else {
      ...
      entries[lineEntries] = (char *) cdf_AllocateMemory (strlen(line)+1, FatalError);
      strcpyX (entries[lineEntries], line, 0);
      if (ib) {
